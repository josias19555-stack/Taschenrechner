...
local function berechneDuenneSchubspannung(Qa, Qb)
    ...
    local function make_path(start_node, first_edge, initial_moment_a, initial_moment_b)
        local path = {id = #paths + 1, items = {}}
            ...
            if node_index then
                for _, edge_index in ipairs(nodes[node_index].edges) do
                    if not edges[edge_index].used then
                        make_path(node_index, edge_index)
                        return true
                    end
                end
            end
    ...
    for node_index, node in ipairs(nodes) do
        if #node.edges == 1 and not edges[node.edges[1]].used then
            make_path(node_index, node.edges[1])
        end
    end
    ...
    local has_free_end = false
    for _, node in ipairs(nodes) do
        if #node.edges == 1 then has_free_end = true; break end
    end
    if symmetric_profile and not has_free_end then
        if not start_at_symmetry_point(preferred_symmetry_points) then
            if not start_at_symmetry_point(fallback_symmetry_points) then
                status = "Warnung: Geschlossenes Profil schneidet keine Symmetrieachse."
            end
        end
    end

    -- Geschlossene Komponenten oder Restkanten ohne freie Enden werden
    -- ansonsten deterministisch mit einem Startwert null orientiert.
...
local function drawShearProfileLegacy(gc)
    ...
    local profile_name = shear_view_mode == 1 and "τ_" .. axis_a .. "-Anteil [MPa]"
        or shear_view_mode == 2 and "τ_" .. axis_b .. "-Anteil [MPa]"
        or shear_view_mode == 3 and "Resultierende Schubspannung: τ_" .. axis_a .. " + τ_" .. axis_b .. " [MPa]"
        or shear_view_mode == 4 and "S" .. axis_a .. " [" .. unit_label(3) .. "]"
        or shear_view_mode == 5 and "S" .. axis_b .. " [" .. unit_label(3) .. "]"
        or shear_view_mode == 6 and "Torsionsanteil [MPa]"
        or shear_view_mode == 7 and "Gesamtschubspannung [MPa]"
        or shear_view_mode == 8 and "Laufvariable s [" .. unit_label(1) .. "]"
        or shear_view_mode == 9 and axis_b .. " * h(" .. axis_b .. ") [" .. unit_label(2) .. "]"
        or shear_view_mode == 10 and axis_a .. " * h(" .. axis_a .. ") [" .. unit_label(2) .. "]"
    ...
    local function profile_value(sample)
        return shear_view_mode == 1 and sample.tau_a
            or shear_view_mode == 2 and sample.tau_b
            or shear_view_mode == 4 and sample.Sy
            or shear_view_mode == 5 and sample.Sz
            or shear_view_mode == 6 and torsionTauAtSample(sample)
            or shear_view_mode == 7 and combinedTauAtSample(sample)
            or shear_view_mode == 8 and (sample.s or 0)
            or shear_view_mode == 9 and select(2, displayedMomentDensity(sample))
            or shear_view_mode == 10 and displayedMomentDensity(sample)
            or sample.tau
    end
    local function profile_display_value(sample)
        local value = profile_value(sample)
        if shear_view_mode == 4 or shear_view_mode == 5 then return display_volume(value) end
        if shear_view_mode == 8 then return display_length(value) end
        if shear_view_mode == 9 or shear_view_mode == 10 then return display_area(value) end
        return value
    end
    local sections = {}
    ...
    for _, section in pairs(sections) do
        ...
        local start_sample = section[1] and section[1].sample
        if start_sample then
            local start_x, start_y = toScreen(start_sample.u, start_sample.v)
...
local function drawShearProfile(gc)
    ...
    gc:setColorRGB(25, 95, 165)
    gc:setPen("medium", "smooth")
    for _, path in ipairs(shear_results.paths) do
        for _, item in ipairs(path.items) do
            if item.p1 and item.p2 then
                local midpoint_u = (item.p1.u + item.p2.u) / 2
                local midpoint_v = (item.p1.v + item.p2.v) / 2
                local direction_u = item.p2.u - item.p1.u
                local direction_v = item.p2.v - item.p1.v
                local length = math.sqrt(direction_u^2 + direction_v^2)
                if length > 1e-9 then
                    local unit_u, unit_v = direction_u / length, direction_v / length
                    local tail_u, tail_v = midpoint_u - unit_u * 0.35, midpoint_v - unit_v * 0.35
                    local tip_u, tip_v = midpoint_u + unit_u * 0.35, midpoint_v + unit_v * 0.35
                    local tail_x, tail_y = toScreen(tail_u, tail_v)
                    local tip_x, tip_y = toScreen(tip_u, tip_v)
                    gc:drawLine(tail_x, tail_y, tip_x, tip_y)
                    local screen_u, screen_v = tip_x - tail_x, tip_y - tail_y
                    local screen_length = math.sqrt(screen_u^2 + screen_v^2)
                    if screen_length > 1e-9 then
                        local arrow_u, arrow_v = screen_u / screen_length, screen_v / screen_length
                        local normal_u, normal_v = -arrow_v, arrow_u
                        gc:drawLine(tip_x, tip_y, tip_x - arrow_u * 5 + normal_u * 3, tip_y - arrow_v * 5 + normal_v * 3)
                        gc:drawLine(tip_x, tip_y, tip_x - arrow_u * 5 - normal_u * 3, tip_y - arrow_v * 5 - normal_v * 3)
                    end
                end
            end
...
local function drawShearSelector(gc, w, h)
    ...
    gc:setFont("sansserif", "r", 9)
    local a, b = axes()
    gc:drawString("1: τ_" .. a .. "-Anteil", left + 8, top + 30)
    gc:drawString("2: τ_" .. b .. "-Anteil", left + 8, top + 48)
    gc:drawString("3: Resultierender Verlauf", left + 8, top + 66)
    gc:drawString("4: S" .. a .. "   5: S" .. b .. "   6: Torsion", left + 8, top + 84)
    gc:drawString("7: Gesamt   8: s   9: " .. b .. "*h(" .. b .. ")", left + 8, top + 100)
    gc:drawString("0: " .. a .. "*h(" .. a .. ")", left + 8, top + 116)
end
...
function on.backspaceKey()
    ...
    elseif #pending > 0 then
        ...
        elseif #massiv_elemente > 0 then
            ...
            -- Loch-Erkennung muss ggf. neu laufen, aber am sichersten:
            for _, e in ipairs(massiv_elemente) do checkIfHole(e) end
            berechneSystem()
