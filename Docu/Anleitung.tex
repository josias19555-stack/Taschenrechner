...
\begin{itemize}
  ...
  \item \emph{$\sigma_x$ aus Kräften}, wenn ihr Kräfte platziert habt.
\end{itemize}
...
mit gedrehten Hauptachsen. Dann kommt eine rote Box mit dem Grund. Ist der Schubmittelpunkt nur
teilweise bestimmt, gibt euch das Programm nur die Koordinate, die wirklich feststeht.
...
\textbf{Und wenn die Symmetrie fehlt?} Dann warnt euch das Programm, und zwar an zwei Stellen:
einmal bei der Schubrechnung selbst (die Zelle wurde an beliebiger Stelle aufgeschnitten, $S$ und
$\tau$ gelten nur bis auf $q_0$), und einmal, wenn ihr einen Verlauf anschaut, dessen Richtung
keine Symmetrieachse hat. Habt ihr z.\,B.\ nur die $z$-Achse als Symmetrieachse, dann sind $S_y$
und $\tau_z$ in Ordnung, $S_z$ und $\tau_y$ aber nicht -- und genau dann kommt die Warnung.
...
\begin{longtable}{p{0.40\linewidth}p{0.55\linewidth}}
  ...
  Schub: Querkraft quer zur Symmetrieachse & Der Zellschubfluss folgt nur aus der Symmetrie; die
    Meldung nennt die erkannten Achsen \\
  Geschlossene Zelle ohne erkannte Symmetrie & $q_0$ bleibt offen, $S$ und $\tau$ gelten nur bis
    auf diesen konstanten Anteil \\
  $S$ bzw.\ $\tau$ in der Richtung ohne Symmetrieachse & Der gezeigte Verlauf ist dort sehr
...
\begin{longtable}{p{0.22\linewidth}p{0.74\linewidth}}
  ...
  \midrule \endhead
  Pfeiltasten & Zelle wählen: links/rechts der Schnitt, oben/unten $\alpha$, $\sigma$, $\tau$ \\
...
\textbf{Spannungsscheibe (\taste{s}):} Die Scheibe ist ein \textbf{Vieleck mit einer Kante je
...
außerhalb der Kante. Decken eure Schnitte nicht den ganzen Umfang ab -- bei zwei senkrechten
Schnitten zum Beispiel --, ergänzt das Programm die Rückseiten, damit die Scheibe geschlossen
ist; diese Kanten sind mit \code{R} gekennzeichnet und tragen dieselben Spannungen.
...
als Punkt mit Radiusstrahl eingezeichnet. Denkt an den Faktor~2: Eine Drehung des Schnitts um
$\alpha$ entspricht im Kreis $2\alpha$, und bei wachsendem $\alpha$ wandern die Punkte im
Uhrzeigersinn.
...
dem Winkel in der Tabelle. Liegt euer Wert gar nicht auf dem Kreis (also
$|\sigma - \sigma_m| > R$), sagt euch das Programm das für diesen Schnitt.

\section{Grenzen und Tricks}

\begin{itemize}
  \item Schnitte \emph{ohne} Winkel helfen beim Bestimmen des Zustands nicht; sie werden erst
        ausgewertet, wenn er aus den anderen Angaben feststeht.
...
\begin{longtable}{p{0.14\linewidth}p{0.82\linewidth}}
  ...
  \textbf{Taste} & \textbf{Funktion} \\
  \midrule \endhead
  Pfeiltasten & Zelle wählen (links/rechts Schnitt, oben/unten $\alpha$, $\sigma$, $\tau$) \\
