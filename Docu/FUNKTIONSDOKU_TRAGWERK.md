...
- **Lösung:** Zeilen mit nur einer Unbekannten werden vorab eliminiert, der Rest mit `exact(simult(A,b))` gelöst; Zahlen gehen exakt ein (`neuZahl`: Bruch, Wurzel oder 12 Stellen). Die exportierten Funktionen sind exakt (keine Rundung); auf dem Rechner bei Bedarf `approx(wneu1(x))`.
...
- **Zweck:** Nach jeder Berechnung Hinweise für die gelbe Box sammeln (`hinweisListe`, `hinweisOffen`; ESC setzt `hinweisQuittiert`, gleiche Hinweise erscheinen erst nach einer Änderung wieder): Temperatur mit To ≠ Tu bei h = 0; Temperatur im symbolischen Modus; im symbolischen Modus Zahl-EI (bzw. -EA) eines nicht starren Stabes ohne beidseitige Gelenke, wenn andere Steifigkeiten (Stab-EI/EA oder Federn) symbolisch sind und die FEM-Endkräfte Biegung (bzw. Normalkraft) zeigen.
