-- Rand-LGS dehnstarr (randDehnstarr): Grenzwert EA -> unendlich fuer Staebe mit Zahlen-EA.
...
fall('1 Portal symbolisch', function()
    ...
    if setup('Zweigelenkrahmen, H und EI symbolisch, EA = 1e10 -> dehnstarr', K, S) then
        check('vne1(x) = H l^2 x/3 - H x^3/12', 'vne1(x)', 'h*l^2*x/3-h*x^3/12')
        check('vne1(l) = H l^3/4', 'vne1(l)', 'h*l^3/4')
        check('vne2(0) = 0 (Stuetzen dehnstarr)', 'vne2(0)', '0')
        check('vne2(l/2) = 0 (antimetrisch)', 'vne2(l/2)', '0')
        check('vne3(0) = -H l^3/4', 'vne3(0)', '-h*l^3/4')
        check('uneu1(x) = EA*u1 = H x', 'uneu1(x)', 'h*x')
        check('uneu3(x) = H (l - x)', 'uneu3(x)', 'h*(l-x)')
        check('uneu2(x) = u2 = H l^3/(4 EI)', 'uneu2(x)', 'h*l^3/(4*ei)')
        for i = 1, 3 do sauber('vne' .. i .. '(x) ohne winzige Brueche', 'vne' .. i .. '(x)') end
        enthaelt('randinfo', 'randinfo', 'dehnstarr')
...
fall('2 Rahmenecke numerisch', function()
    ...
    if setup('Rahmenecke, q = 1, EI = 1e8, EA = 1e10 -> dehnstarr', K, S) then
        ...
        check('vne1(x) = x^2/16', 'vne1(x)', 'x^2/16')
        check('vne2(0) = 0', 'vne2(0)', '0')
        check('vne2(1) = 17/96', 'vne2(1)', '17/96')
        check('vne2(2) = 0 (Rolle)', 'vne2(2)', '0')
        check('vne2(x) Formel', 'vne2(x)', '(2-x)^4/24-15/16*(2-x)^3/6-7/24*x+7/12')
...
fall('3 Rahmenecke symbolisch', function()
    ...
    if setup('Rahmenecke, q symbolisch, EI/EA numerisch -> dehnstarr', K, S) then
        check('vne1(x) = q l^2 x^2/16', 'vne1(x)', 'q*l^2*x^2/16')
        check('vne2(l) = 17/96 q l^4', 'vne2(l)', '17/96*q*l^4')
        check('vne2(0) = 0', 'vne2(0)', '0')
        check('vne2(2l) = 0', 'vne2(2*l)', '0')
        check('uneu1(x) = -17/16 q l x', 'uneu1(x)', '-17*q*l*x/16')
        sauber('vne2(x) ohne winzige Brueche', 'vne2(x)')
    end
end)

fall('4 Festlager-Festlager', function()
    -- Horizontallast zwischen zwei Festlagern: N verteilt sich nach EA (1:2). Mit 1/EA = 0
    -- waere das LGS singulaer; der Grenzwert liefert die Aufteilung.
    local K = { pinned(node(0)), node(1), pinned(node(2)) }
    K[2].last_x = 1
    local S = { T.bar(1, 2, { EI = 1e8, EA = 1e10 }), T.bar(2, 3, { EI = 1e8, EA = 2e10 }) }
    if setup('Festlager-Festlager, F = 1 in der Mitte, EA 1:2', K, S) then
        check('uneu1(x) = N1 x = x/3', 'uneu1(x)', 'x/3')
        check('uneu2(x) = 2/3 (1 - x)', 'uneu2(x)', '2/3*(1-x)')
        check('vne1(x) = 0', 'vne1(x)', '0')
        enthaelt('randinfo ohne u-Hinweis', 'randinfo', '=u', false)
    end
end)
...
fall('6 Temperatur gehalten', function()
    ...
    enthaelt('randbed mit Meldung', 'randbed', 'Dehnstarr')
    T.pruef(type(num('vne1(1)')) ~= 'number', 'vne1 nicht definiert')
...
fall('7 Schalter aus', function()
    ...
    if setup('Zweigelenkrahmen, dehnstarr aus (EA = 1e10 endlich)', K, S, {}) then
        enthaelt('randinfo', 'randinfo', 'dehnbar')
        enthaelt('vne2(x) mit EA-Anteil', 'vne2(x)', '10000000000')
...
fall('8 EA symbolisch bleibt dehnbar', function()
    ...
    if setup('Zweigelenkrahmen, EA symbolisch -> nicht dehnstarr', K, S) then
        enthaelt('randinfo', 'randinfo', 'dehnbar')
        enthaelt('vne1(x) enthaelt ea', 'vne1(x)', 'ea')
    end
end)
