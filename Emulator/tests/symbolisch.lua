-- Symbolischer Modus: Rand-LGS (vne/uneu) gegen geschlossene Handformeln.
...
local function setup(title, K, S, opts)
    ...
    for i = 1, #S do T.info('   vne' .. i .. '(x) = ' .. str('vne' .. i .. '(x)')) end
    return true
end
...
fall('1 Einfeld q', function()
    if setup('Einfeldtraeger L=2l, q', { pinned(node(0)), roller(node(2)) }, { bar(1, 2, { q = 1, q_str = 'q' }) }) then
        check('vne1(l) = 5/24 q l^4', 'vne1(l)', '5/24*q*l^4')
        check('vne1(x) Formel', 'vne1(x)', 'q*x*((2*l)^3-2*(2*l)*x^2+x^3)/24')
...
fall('2 Kragarm F', function()
    ...
    if setup('Kragarm L=l, F an der Spitze', K, { bar(1, 2) }) then
        check('vne1(l) = F l^3/3', 'vne1(l)', 'F*l^3/3')
        check('vne1(x) = F x^2(3l-x)/6', 'vne1(x)', 'F*x^2*(3*l-x)/6')
    end
end)

fall('3 Eingespannt q', function()
    if setup('Beidseitig eingespannt L=2l, q', { fixed(node(0)), fixed(node(2)) }, { bar(1, 2, { q = 1, q_str = 'q' }) }) then
        check('vne1(l) = q(2l)^4/384', 'vne1(l)', 'q*(2*l)^4/384')
        check('vne1(x) = q x^2 (L-x)^2/24', 'vne1(x)', 'q*x^2*(2*l-x)^2/24')
    end
end)
...
fall('4 Gerber F', function()
    ...
    if setup('Gerbertraeger: Einsp. 0, Gelenk l, F bei 3l/2, Rolle 2l', K, { bar(1, 2), bar(2, 3), bar(3, 4) }) then
        check('vne1(l) = F l^3/6', 'vne1(l)', 'F*l^3/6')
        check('vne2(l/2) = 5/48 F l^3', 'vne2(l/2)', '5/48*F*l^3')
        check('vne3(0) = 5/48 F l^3', 'vne3(0)', '5/48*F*l^3')
        check('vne3(l/2) = 0', 'vne3(l/2)', '0')
        check('vne1(x) = F x^2(3l-x)/12', 'vne1(x)', 'F*x^2*(3*l-x)/12')
    end
end)
...
fall('5 Drehfeder', function()
    ...
    if setup('Einfeldtraeger l, Drehfeder cm=EI/l links, q', K, { bar(1, 2, { q = 1, q_str = 'q' }) }) then
        ...
        check('vne1(l/2) = 17/1536 q l^4', 'vne1(l/2)', '17/1536*q*l^4')
        check('vne1(x) Formel', 'vne1(x)', 'q*x*(l^3-2*l*x^2+x^3)/24 - q*l^2/32*x*(l-x)*(2*l-x)/(6*l)')
    end
    ...
    if setup('Einfeldtraeger l, Drehfeder cm=c links, q', K2, { bar(1, 2, { q = 1, q_str = 'q' }) }) then
        ...
        checkNum('vne1(l/2) (c=2,EI=3,l=5,q=7)', 'vne1(l/2)', '5/384*q*l^4 - (c*q*l^3/(8*(3*EI+c*l)))*l^2/16', { c = 2, ei = 3, l = 5, q = 7 })
        checkNum('vne1(l/4)', 'vne1(l/4)', 'q*l/4*(l^3-2*l*(l/4)^2+(l/4)^3)/24 - (c*q*l^3/(8*(3*EI+c*l)))*(l/4)*(l-l/4)*(2*l-l/4)/(6*l)', { c = 2, ei = 3, l = 5, q = 7 })
    end
end)
...
fall('6 Rahmen H', function()
    ...
    if setup('Zweigelenkrahmen h=L=l, H an Ecke, EA symbolisch', K, S) then
        ...
        checkNum('vne1(l)|EA=1e15 = H l^3/4', 'vne1(l)', 'H*l^3/4', { ea = 1e15, ei = 2, l = 3, h = 5 })
        checkNum('vne3(0)|EA=1e15 = -H l^3/4', 'vne3(0)', '-H*l^3/4', { ea = 1e15, ei = 2, l = 3, h = 5 })
        checkNum('vne2(0)|EA=1e15 = 0', 'vne2(0)', '0', { ea = 1e15, ei = 2, l = 3, h = 5 })
        checkNum('vne2(l/2)|EA=1e15 = 0 (antimetrisch)', 'vne2(l/2)', '0', { ea = 1e15, ei = 2, l = 3, h = 5 })
    ...
    if setup('Zweigelenkrahmen, EA=1e9 numerisch', K2, S2) then
        checkNum('vne1(l) ~ H l^3/4', 'vne1(l)', 'H*l^3/4', { ei = 2, l = 3, h = 5 })
        checkNum('vne3(0) ~ -H l^3/4', 'vne3(0)', '-H*l^3/4', { ei = 2, l = 3, h = 5 })
    end
end)
...
fall('7 EI numerisch', function()
    ...
    if setup('Einspannung-Rolle 2l, q symbolisch, EI numerisch', { fixed(node(0)), roller(node(2)) }, S2) then
        check('vne1(x) = q x^2 (L-x)(3L-2x)/48', 'vne1(x)', 'q*x^2*(2*l-x)*(3*2*l-2*x)/48')
    end
end)
...
fall('8 Grenzen', function()
    local K = { fixed(node(0)), node(1) }
    K[2].last_y, K[2].last_y_str = 1, 'F'
    abbruch('Temperatur im symbolischen Modus', K, { bar(1, 2, { To = 0, Tu = 100, h = 0.2, alpha = 1e-5 }) }, 'Temperatur')
...
fall('9 Koordinaten l', function()
    ...
    if setup('Einfeldtraeger 2l (x_str l, 2*l), F mittig', K, { bar(1, 2), bar(2, 3) }) then
        check('vne1(l) = F (2l)^3/48', 'vne1(l)', 'F*(2*l)^3/48')
        check('vne1(x) = F x (3L^2-4x^2)/48', 'vne1(x)', 'F*x*(3*(2*l)^2-4*x^2)/48')
    end
    local K2 = { fixed(node(0, 0, '0', '0')), node(3, 0, 'a', '0') }
    K2[2].last_y, K2[2].last_y_str = 1, 'F'
    if setup('Kragarm Laenge a, F', K2, { bar(1, 2) }) then
        check('vne1(a) = F a^3/3', 'vne1(a)', 'F*a^3/3')
    end
    ...
    if setup('Kragarm 2 m bei Raster 0,5 -> L = 4l', K3, { bar(1, 2) }, { raster = 0.5 }) then
        check('vne1(4l) = F (4l)^3/3', 'vne1(4*l)', 'F*(4*l)^3/3')
    end
    ...
    if setup('Schraeger Kragarm (0,0)-(1,1), zahlenFormat 2', K4, { bar(1, 2) }, { zahlenFormat = 2 }) then
        ...
        check('vne1(sqrt(2) l) = 2/3 F l^3', 'vne1(sqrt(2)*l)', '2/3*F*l^3')
    end
end)
...
fall('10 Weitere Lasten', function()
    if setup('Einfeldtraeger l, Dreieckslast 0 -> q', { pinned(node(0)), roller(node(1)) },
        ...
        check('vne1(x) Dreieck', 'vne1(x)', 'q*x*(7*l^4-10*l^2*x^2+3*x^4)/(360*l)')
    end
    ...
    if setup('Einfeldtraeger l, Knotenmoment M0 rechts', K3, { bar(1, 2) }) then
        check('vne1(l/2) = M0 l^2/16', 'vne1(l/2)', 'M0*l^2/16')
    end
    local K4 = { fixed(node(0)), node(1) }
    K4[2].cy, K4[2].cy_str = 1, 'c'
    K4[2].last_y, K4[2].last_y_str = 1, 'F'
    if setup('Kragarm l, Feder cy=c am Ende, F', K4, { bar(1, 2) }) then
        checkNum('vne1(l) = F EI l^3/(c l^3+3EI)', 'vne1(l)', 'F*EI*l^3/(c*l^3+3*EI)', { c = 2, ei = 3, l = 5, f = 7 })
    end
